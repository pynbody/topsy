...
fn vertex_main(@location(0) instanceStart : vec4<f32>,
    ...
    var instanceStartTransformed : vec2<f32> = (render_params.transform * instanceStart).xy;
    var instanceEndTransformed : vec2<f32> = (render_params.transform * instanceEnd).xy;
    ...
    switch vertexIndex {
        case 0u {
           output.pos = vec4<f32>(instanceStartTransformed - normalToLine * render_params.width * 0.5, 0.0, 1.0);
        }
        case 1u: {
           output.pos = vec4<f32>(instanceStartTransformed + normalToLine * render_params.width * 0.5, 0.0, 1.0);
        }
        case 2u: {
           output.pos = vec4<f32>(instanceEndTransformed - normalToLine * render_params.width * 0.5, 0.0, 1.0);
        }
        case 3u: {
           output.pos = vec4<f32>(instanceEndTransformed + normalToLine * render_params.width * 0.5, 0.0, 1.0);
        }
